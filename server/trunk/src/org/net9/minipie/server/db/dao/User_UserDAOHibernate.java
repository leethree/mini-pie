 ...
 *
 */
public class User_UserDAOHibernate extends GenericHibernateDAO<User2User, Id> implements
	...
	public Collection<CommonListEntry> selectSharedUser(Long userId,
		...
		if(binds1!=null){
			...
			while(iter1.hasNext()){
				...
				try {
					result.add(new CommonListEntry(user2.getId(), user2.getUserName(),
							user2.getImageURL()));
		...
		if(binds2!=null){
			...
			while(iter2.hasNext()){
				...
				try {
					result.add(new CommonListEntry(user1.getId(), user1.getUserName(),
							user1.getImageURL()));
				} catch (DataFormatException e) {
					throw new ServerErrorException(e.getMessage());
				}
