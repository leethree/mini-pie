 ...
 * 
 */
public class Tag_ContactDAOHibernate extends
		GenericHibernateDAO<Tag2Contact, Id> implements Tag_ContactDAO,
		Tag_ContactStorage {

	/*
	 * (non-Javadoc)
	 * 
	 * @see org.net9.minipie.server.db.dao.Tag_ContactDAO#add(java.lang.Long,
	 * java.lang.Long)
	 */
	public void add(Long tagId, Long contactId) {
		...
		try {
			...
			throw new NotFoundException("there is no contact with id: "
					+ contactId);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.net9.minipie.server.db.dao.Tag_ContactDAO#del(java.lang.Long,
	 * java.lang.Long)
	 */
	public void del(Long tagId, Long contactId) {
		...
		try {
			...
			throw new NotFoundException(
					"there is no tag 2 contact with tag id: " + tagId
							+ " and with contact id: " + contactId);
		}
		begin();
	 ...
	 * 
	 * @see
	 * org.net9.minipie.server.db.dao.Tag_ContactDAO#selectTaggedContact(java
	 * .lang.Long)
	 */
		...
		while (iter.hasNext()) {
			...
			try {
				result.add(new BasicContact(contact.getId(), contact.getName(),
						contact.getImage(), contact.getNickName(), contact
								.getGender(),
						(contact.getBirthday() != null) ? contact.getBirthday()
								.toString() : null, contact.getNotes(), contact
								.getRelationship(),
						(contact.getOwner() != null) ? contact.getOwner()
								.getId().longValue() : 0, (contact
								.getShadowOf() != null) ? contact.getShadowOf()
								.getId().longValue() : 0,
